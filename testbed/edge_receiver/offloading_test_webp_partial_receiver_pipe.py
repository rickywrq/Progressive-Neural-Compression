...
def serial_read_and_response(ser:serial.Serial, classifier, idx, response=True):
    ...
    if error == b'1': 
        print("-=-=-=-=-=-=-=-= Partial -=-=-=-=-=-=-=-=-")
        x_thread = threading.Thread(target=tf_inference_partial, args=(data, idx))
        x_thread.start()
    elif error == b'0':
        tmp_file2 = io.BytesIO(data)
        image_data = np.array(Image.open(tmp_file2))/255.0
        image_data = np.expand_dims(image_data, 0)
        x_thread = threading.Thread(target=tf_inference, args=(image_data, idx))
        x_thread.start()
        print("- - - - - - - - - OK - - - - - - - -")
    return seq
...
if __name__ == "__main__":
    ...
    while i < args.n:
        try:
            ...
            serial_read_and_response(ser, classifier, i,  response=False)
            print(seqrtr)
            i = seqrtr
            # assert seq == i
    ...
    p_count = np.count_nonzero(recv_status == 2)
    total_imgs = recv_status.shape[0]
    print(list_l[:args.n])
    # print(result_all)
    # print(recv_status)
    print("{}/{}={:.4f}".format(valid_count, total_imgs, valid_count/total_imgs))
    print("{}/{}={:.4f}".format(p_count, total_imgs, p_count/total_imgs))
